...
### V3.0 基本功能完善 ###
### V5.0 稳定版 slab 逻辑 + StructureGraph/jimage 判键 + V_old + 最优切面二维模型导出 ###

"""
配置区说明
========

1. 直接运行 `python version_5.0.py` 时，会读取脚本顶部的 `SCRIPT_RUN_CONFIG`。
...
DEFAULT_2D_MODEL_VACUUM_SIZE = 15.0
...
# 修改这里即可控制输入参数
# 你平时只需要改这一个字典。
# 如果直接运行 `python version_5.0.py`，程序会自动读取这里的设置。
...
CONFIG_HELP = """
默认情况下，直接运行 `python version_5.0.py` 会读取脚本顶部的 `SCRIPT_RUN_CONFIG`。
...
def cyclic_fractional_distance(value_a, value_b):
    """计算两个 [0, 1) 分数坐标在周期边界下的最短距离。"""
    delta = abs(float(value_a) - float(value_b))
    return min(delta, 1.0 - delta)


def snap_shift_to_termination(raw_shift, candidate_shifts):
    """把原始 shift 吸附到最近的合法 termination shift。"""
    raw_shift = float(raw_shift) % 1.0
    if not candidate_shifts:
        return raw_shift

    return min(
        (float(shift) % 1.0 for shift in candidate_shifts),
        key=lambda shift: (cyclic_fractional_distance(raw_shift, shift), shift),
    )
...
def structure_classification(h, k, l, identifier, chemical_formula):
    ...
    if not target_folder.exists():
        ...
        for fname in [f"POSCAR_{h}_{k}_{l}.vasp", f"POSCAR_2D_{h}_{k}_{l}.vasp"]:
...
def generate_2d_model_from_best_plane(
    structure_path,
    h,
    k,
    l,
    best_plane_height,
    vacuum_size=DEFAULT_2D_MODEL_VACUUM_SIZE,
):
    """
    根据最低键密度对应的最佳切面，生成最终二维结构模型。

    思路：
    1. 把评估 slab 中的最佳切面高度换算成对应的 termination shift；
    2. 将该 shift 吸附到最近的合法 termination；
    3. 用相同 hkl 从原始 bulk 重新生成一个更薄、上下表面都落在该 termination 的二维 slab。

    这样导出的 2D 模型不会受评估 slab 额外层数影响，上下表面也都对应之前最低键密度的位置。
    """
    if best_plane_height is None:
        return None
    ...
    probe_generator = SlabGenerator(
        ...
        center_slab=False,
        min_slab_size=1.0,
        min_vacuum_size=DEFAULT_MIN_VACUUM_SIZE,
    )
    projected_height = float(probe_generator._proj_height)
    if projected_height <= PLANE_TOLERANCE:
        return None

    candidate_slabs = probe_generator.get_slabs(filter_out_sym_slabs=False)
    candidate_shifts = [float(slab.shift) % 1.0 for slab in candidate_slabs]
    raw_shift = (float(best_plane_height) / projected_height) % 1.0
    snapped_shift = snap_shift_to_termination(raw_shift, candidate_shifts)

    final_generator = SlabGenerator(
        structure,
        miller_index,
        center_slab=True,
        min_slab_size=projected_height,
        min_vacuum_size=float(vacuum_size),
    )
    two_dimensional_slab = final_generator.get_slab(shift=snapped_shift)
    orthogonal_two_dimensional_slab = two_dimensional_slab.get_orthogonal_c_slab()

    output_file_name = f'POSCAR_2D_{h}_{k}_{l}.vasp'
    if os.path.exists(output_file_name):
        os.remove(output_file_name)
    Poscar(orthogonal_two_dimensional_slab).write_file(output_file_name)

    atoms = read(output_file_name)
    positions = atoms.get_positions()
    zmin = float(np.min(positions[:, 2]))
    zmax = float(np.max(positions[:, 2]))
    cell = atoms.get_cell()
    actual_slab_thickness = float(zmax - zmin)
    vacuum_thickness = float(cell[2, 2] - actual_slab_thickness)

    return {
        'two_d_model_file_name': output_file_name,
        'two_d_target_shift': float(snapped_shift),
        'two_d_raw_shift': float(raw_shift),
        'two_d_period_height': projected_height,
        'two_d_slab_thickness': actual_slab_thickness,
        'two_d_vacuum_thickness': vacuum_thickness,
    }
...
def cleanup_temp_files(h, k, l):
    file_candidates = [f'POSCAR_{h}_{k}_{l}.vasp', f'POSCAR_2D_{h}_{k}_{l}.vasp']
...
def initialize_method_result(method_name, selection_metric):
    ...
    return {
        ...
        'scan_event_count': 0,
        'two_d_model_file': '',
        'two_d_raw_shift': None,
        'two_d_target_shift': None,
        'two_d_period_height': None,
        'two_d_slab_thickness': None,
        'two_d_vacuum_thickness': None,
    }
...
def write_graph_result_to_dataframe(df, index, graph_result, slab_duplicate_of=''):
    ...
    df.at[index, 'best_plane_index'] = graph_result['plane_index']
    df.at[index, 'best_plane_height'] = graph_result['plane_height']
    df.at[index, 'best_plane_shift'] = graph_result['plane_shift']
    df.at[index, 'two_d_model_file'] = graph_result.get('two_d_model_file', '')
    df.at[index, 'two_d_raw_shift'] = graph_result.get('two_d_raw_shift')
    df.at[index, 'two_d_target_shift'] = graph_result.get('two_d_target_shift')
    df.at[index, 'two_d_period_height'] = graph_result.get('two_d_period_height')
    df.at[index, 'two_d_slab_thickness'] = graph_result.get('two_d_slab_thickness')
    df.at[index, 'two_d_vacuum_thickness'] = graph_result.get('two_d_vacuum_thickness')
...
def update_method_result(method_result, score, area_density, volume_density, mean_volume,
                         mean_distances, area, bond_pairs, plane_index, plane_height, plane_shift):
    """若当前平面优于历史最优结果，则更新记录。"""
    if score < method_result['best_score']:
        ...
        method_result['plane_shift'] = plane_shift
...
def analyze_slab_file(
    ...
    try:
        bulk_structure = Structure.from_file(structure_path)
        probe_generator = SlabGenerator(
            bulk_structure,
            [int(h), int(k), int(l)],
            center_slab=False,
        ...
        plane_repeat_height = float(probe_generator._proj_height)
    except Exception:
        plane_repeat_height = float(init_d)
    if plane_repeat_height <= PLANE_TOLERANCE:
        plane_repeat_height = 1.0
    ...
    graph_result['repeat_period'] = plane_repeat_height
    ...
    for plane_index, plane_height in enumerate(scan_heights):
        ...
        plane_shift = float((plane_height / plane_repeat_height) % 1.0)
        ...
        record['graph_plane_shift'] = plane_shift
    ...
    if graph_result['plane_height'] is not None and graph_result['bond_pairs']:
        two_d_model = generate_2d_model_from_best_plane(
            structure_path,
            h,
            k,
            l,
            graph_result['plane_height'],
        )
        if two_d_model is not None:
            graph_result['plane_shift'] = float(two_d_model['two_d_raw_shift'])
            graph_result['two_d_model_file'] = two_d_model['two_d_model_file_name']
            graph_result['two_d_raw_shift'] = two_d_model['two_d_raw_shift']
            graph_result['two_d_target_shift'] = two_d_model['two_d_target_shift']
            graph_result['two_d_period_height'] = two_d_model['two_d_period_height']
            graph_result['two_d_slab_thickness'] = two_d_model['two_d_slab_thickness']
            graph_result['two_d_vacuum_thickness'] = two_d_model['two_d_vacuum_thickness']
...
def build_args_from_script_config():
    """
    当用户直接运行 `python version_5.0.py` 时，使用脚本顶部配置区生成参数。
    """
